...
class MyDataset(torch.utils.data.Dataset):
    def __init__(self, transform):
        images = np.load('./datasets/cifar_adv1_image.npy')
        labels = np.load('./datasets/cifar_adv1_label.npy')
        assert labels.min() >= 0
...
def attack(models, x, y, iter=10, eps=0.001):
    ...
    # # adversary_resnet = PGD(models[0])
    # adversary_densenet = PGD(models[1])
    # attack_params = {'epsilon': 0.1, 'clip_max': 10000.0, 'clip_min': -10000.0, 'num_steps': 40, 'print_process': False}
    # # x = adversary_resnet.generate(x, y, **attack_params)
    # x = adversary_densenet.generate(x, y, **attack_params)
    # return x
    ...
    # atk_resnet = CW(norm_resnet, c=1, kappa=0, steps=100, lr=0.01)
    # atk_densenet= CW(norm_densenet, c=1, kappa=0, steps=100, lr=0.01)
    # adv_images = atk_resnet(x, y)
    # return adv_images
...
resnet.load_state_dict(torch.load('./checkpoints/resnet_adv1.pth')['state_dict'])
resnet.eval()
densenet = load_model('densenet121').cuda()
densenet.load_state_dict(torch.load('./checkpoints/densenet_adv1.pth')['state_dict'])
densenet.eval()
...
np.save('cifar_PGD_d_densenet_image.npy', images_adv)
np.save('cifar_PGD_d_densenet_label.npy', labels_adv)